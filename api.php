 ...
 *
 * Forked and modified: thad@blockchainindusrties.io
 * https://github.com/kantmakm/empapi
 **/
...
class GenericDB
{
    ...
    public function selectSingle(ReflectedTable $table, array $columnNames, String $id) /*: ?array*/
    {
        ...
        if ($tableName == 'employees')
            {
                $sql = 'SELECT ' . $selectColumns . ' FROM "' . $tableName . '" ' . $whereClause . ' and status = \'ACTIVE\'';
            }
        else
            {
    ...
    public function deleteSingle(ReflectedTable $table, String $id)
    {
        ...
        if ($tableName == 'employees')
            {
                $sql = 'UPDATE "' . $tableName . '" ' . 'set status = \'INACTIVE\'' . $whereClause;
            }
        else
            {
